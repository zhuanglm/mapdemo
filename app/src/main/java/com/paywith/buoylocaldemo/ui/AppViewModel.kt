...
class AppViewModel @Inject constructor(
    ...
    private fun loadOffers() {
        viewModelScope.launch {
            ...
            try {
                val domainOffers: List<Offer> = getOffers(searchQuery)
                val uiOffers: List<OfferUiModel> =
                    domainOffers.map { it.toOfferUiModel(userCoords) }

                _offers.value = uiOffers
            } catch (e: Exception) {
                Log.e("OffersDebug", "Failed to load offers: ${e.message}", e)
    ...
    private fun loadMockOffers() {
        viewModelScope.launch {
            ...
            val uiOffers: List<OfferUiModel> = mockOffers.map { it.toOfferUiModel(userCoords) }
            _offers.value = uiOffers
        }
    }
